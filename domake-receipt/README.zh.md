...
当前公开版本使用实验性浏览器英文图片 OCR、保守字段整理、失败恢复与真实图片测试入口。云端 AI 接通前，PDF 仍为预览及手工填写；3 份预设示例保留明确演示标识。确认后的记录和原件存 IndexedDB，仅在此浏览器可用，没有共享云端数据库或客户登录。

**2026-09-12 更新：** 已按用户要求开始真实 AI 接入。云端提取接口、固定输出规范、中英文识别入口、试用访问码、原件重新识别和再次确认更新已实现。前端 81 项自动检查和独立接口 11 项检查通过；它们使用受控回复，不能代表 Gemini 准确率。OpenRouter 登录和 API Key 创建尚未完成，`AI_ENDPOINT` 暂留空，没有真实模型调用。详见[模型推荐和接入状态](docs/cloud-ai.zh.md)。此前截图报告仍对应本地 OCR／手工流程。
